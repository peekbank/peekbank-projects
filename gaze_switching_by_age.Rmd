...
library(here)
library(data.table)
# Mira apology: I learned R using data.table and it's my default even though I'm sure tidyverse is better, so apologies for weird code! I'll try to use as much tidyverse code and use chatGPT to translate any weird code into normal person R code down the line :) 
library(ggplot2)
library(dplyr)
library(tidyr)
```
...
# select adams_marchman dataset
adams_marchman_df <- aoi_data_joined %>%
...
# age
adams_marchman_df = adams_marchman_df %>%
  group_by(subject_id) %>%
  mutate(
...
# plotting the gaze patterns for the 4 administrations (as separate columns)
# for each participant as rows
plot = ggplot(adams_marchman_df,
   	aes(x = t_norm,
       	y = factor(subject_id),
       	color = aoi)) +
  facet_grid(cols = vars(new_admin_id))+
  geom_point(size = 1) +  # One dot per look
  labs(x = "Timestamp", y = "Subject", color = "Look") +
  theme_minimal()
...
trial_switches <- adams_marchman_df %>%
